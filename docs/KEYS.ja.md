...
**このファイルが正本。** キーマップは意図的に小さい。キーが空いているからではなく、
無いと困ると分かったものだけを足す。
...
| **+ / −** | ズームイン / アウト | |
| **0** | ズーム全体表示 | |
| **Enter** | 選択区間のプロンプトへフォーカス | **自動で入るのはこれだけ**（または textarea を自分でクリック）。**入力中はショートカット無効**（Del で枠を消さない） |
...
| 区間枠クリック | その区間を選択（ハイライト） | **プロンプトにフォーカスしない・ヘッドも動かさない**。ピン閉じも同様。Space など音楽操作はそのまま |
...
- 2026-08-04: 枠クリック／ピン閉じでプロンプト自動フォーカスしない（Space が使える）。文字編集は Enter か textarea クリック。枠クリックで t0 シークもしない。
