...
消化（digest）は曲を読んで、区間の**候補**を出す。下の金色の帯がそれで、ここでは 21 本、
librosa の novelty 検出による。この時点では何も決まっていない。操作は **Apply section
drafts** というボタンで、採用しても、置き換えても、全部無視して自分でピンを打ってもいい。

![消化後。波形の上に区間候補が乗った状態](docs/media/digest-after.png)

この区別が設計の全部。解析は手がかりを出し、切るのは人。詳細は
[DECISIONS.md](docs/DECISIONS.md)。

## 区間ひとつを演出する

![区間エディタ。作者のプロンプト、区間モード、任意の Feel、テイク、そしてモデルへ送られた時間窓](docs/media/segment.png)

このプロジェクトが考えていることの大半が、この1枚に写っている。

大きな入力欄は作者のもので、画を決めているのはここだけ。その上にある機械の寄与は
**AI emotion (reference)** と明示されている — 4語、提示されるだけで、無視してよい。
**Segment mode** はここでは `Hold`、同じ場面と画面構成という意味で、だからカメラ固定を
掛けても安全になる。`Shift` と `Motion` は逆のことを言う。**Feel** の2本は任意で、
作者が触らなければ未設定のまま。

生成ボタンは **`Generate another take · ~4 API calls`** と出る。この区間が 15.4 秒で、
単位が5秒だから。下のテイクには `4×5s`、連鎖した4本と記録されている。そのうち1本が
外れたときは **Partial regen** が番号付きの部分 — `0 1 2 3` — だけを撮り直すので、
途中の1秒のために通った3本を捨てずに済む。
